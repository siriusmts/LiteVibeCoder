...
TOOLS = [
    ...
    {"name": "verify_published_bot", "description": "Run a model-defined coverage-driven black-box test plan against the published bot. Decide how many named cases are needed from the task's distinct observable paths. A case is either one independent message or ordered steps in one shared session for stateful behavior; use text, positive regex, and forbidden-regex assertions to prove every path before completion.", "inputSchema": {"type": "object", "properties": {"tests": {"type": "array", "minItems": 1, "items": {"type": "object", "properties": {"name": {"type": "string"}, "message": {"type": "string"}, "expectContains": {"type": "array", "items": {"type": "string"}}, "expectRegex": {"type": "array", "items": {"type": "string"}}, "forbidRegex": {"type": "array", "items": {"type": "string"}}, "expectButtons": {"type": "array", "items": {"type": "string"}}, "expectCommand": {"type": "string"}, "steps": {"type": "array", "minItems": 1, "items": {"type": "object", "properties": {"message": {"type": "string"}, "expectContains": {"type": "array", "items": {"type": "string"}}, "expectRegex": {"type": "array", "items": {"type": "string"}}, "forbidRegex": {"type": "array", "items": {"type": "string"}}, "expectButtons": {"type": "array", "items": {"type": "string"}}, "expectCommand": {"type": "string"}}, "required": ["message"]}}}, "required": ["name"]}}}, "required": ["tests"]}, "annotations": {"x-vibe-role": "verification"}},
]
