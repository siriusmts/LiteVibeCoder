...
TOOLS = [
    ...
    {"name": "verify_published_bot", "description": "Run a coverage-driven black-box test plan. Each case is exactly one shape: independent {name,message,expectContains?,expectRegex?,forbidRegex?,expectButtons?,expectCommand?}; or stateful {name,steps:[{message, assertions?},...]}. Never combine message and steps. Every message is non-empty. Assert only user-requested behavior.", "inputSchema": {"type": "object", "properties": {"tests": {"type": "array", "minItems": 1, "items": {"type": "object"}}}, "required": ["tests"]}, "annotations": {"x-vibe-role": "verification"}},
]
