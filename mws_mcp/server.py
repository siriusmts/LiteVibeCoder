...
TOOLS = [
    ...
    {"name": "verify_published_bot", "description": "Run a model-defined coverage-driven black-box test plan against the published bot. Every case needs a non-empty name and exactly one of message or non-empty steps; never send both. Use steps only for a stateful path. Assert user-requested behavior, not arbitrary generated wording such as an exact joke.", "inputSchema": {"type": "object", "properties": {"tests": {"type": "array", "minItems": 1, "items": {"type": "object", "properties": {"name": {"type": "string"}, "message": {"type": "string"}, "expectContains": {"type": "array", "items": {"type": "string"}}, "expectRegex": {"type": "array", "items": {"type": "string"}}, "forbidRegex": {"type": "array", "items": {"type": "string"}}, "expectButtons": {"type": "array", "items": {"type": "string"}}, "expectCommand": {"type": "string"}, "steps": {"type": "array", "minItems": 1, "items": {"type": "object", "properties": {"message": {"type": "string"}, "expectContains": {"type": "array", "items": {"type": "string"}}, "expectRegex": {"type": "array", "items": {"type": "string"}}, "forbidRegex": {"type": "array", "items": {"type": "string"}}, "expectButtons": {"type": "array", "items": {"type": "string"}}, "expectCommand": {"type": "string"}}, "required": ["message"]}}}, "required": ["name"]}}}, "required": ["tests"]}, "annotations": {"x-vibe-role": "verification"}},
]
