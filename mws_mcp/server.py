...
TOOLS = [
    ...
    {"name": "verify_published_bot", "description": "Run a model-defined coverage-driven black-box test plan against the published bot. Decide how many named cases are needed from the task's distinct observable paths; use this to prove them before completion.", "inputSchema": {"type": "object", "properties": {"tests": {"type": "array", "minItems": 1, "items": {"type": "object", "properties": {"name": {"type": "string"}, "message": {"type": "string"}, "expectContains": {"type": "array", "items": {"type": "string"}}, "expectButtons": {"type": "array", "items": {"type": "string"}}, "expectCommand": {"type": "string"}}, "required": ["name", "message"]}}}, "required": ["tests"]}, "annotations": {"x-vibe-role": "verification"}},
]
