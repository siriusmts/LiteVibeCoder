...
TOOLS = [
    ...
    {"name": "test_published_bot", "description": "Send one live engine message. The result includes sessionId, visible reply, buttons, commands, awaitingUser, and compact engineErrors. Reuse sessionId for the next conversational turn; if buttons are shown, choose one of those labels as the next message. Optional assertions are evaluated on this response.", "inputSchema": {"type": "object", "properties": {"message": {"type": "string"}, "sessionId": {"type": "string"}, "expectContains": {"type": "array", "items": {"type": "string"}}, "expectRegex": {"type": "array", "items": {"type": "string"}}, "forbidRegex": {"type": "array", "items": {"type": "string"}}, "expectButtons": {"type": "array", "items": {"type": "string"}}, "expectCommand": {"type": "string"}}}},
    {"name": "verify_published_bot", "description": "Run a coverage-driven black-box test plan. Each case is exactly one shape: independent {name,message,assertions...}; or stateful {name,steps:[{message, assertions...},...]}. Never combine message and steps. Every message and every step needs at least one non-empty observable assertion (contains, regex, forbidden regex, button, or command). Assert only user-requested behavior.", "inputSchema": {"type": "object", "properties": {"tests": {"type": "array", "minItems": 1, "items": {"type": "object"}}}, "required": ["tests"]}, "annotations": {"x-vibe-role": "verification"}},
]
...
def call(name: str, args: dict[str, Any]) -> Any:
    ...
    if name == "test_published_bot": return RUNTIME.engine_test(args.get("message"), args.get("expectContains"), args.get("expectButtons"), args.get("expectCommand"), args.get("sessionId"), expect_regex=args.get("expectRegex"), forbid_regex=args.get("forbidRegex"))
