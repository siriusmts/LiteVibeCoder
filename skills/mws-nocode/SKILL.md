...
Response mappings are exposed as lists, including a mapping that selects one JSON value. For a possibly empty API result, first route with a short condition such as `session.result_title` (truthiness) to the success node; leave the following blocks in that node as the empty-result path. Do not index `[0]` in that condition: an empty list makes the engine fail. In the success answer, render the scalar with `{{session.result_title[0]}}`. This makes the same graph handle a populated list and an empty response without a script.
